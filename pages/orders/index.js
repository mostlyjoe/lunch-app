...
export default function OrdersPage() {
    ...
    const confirmDelete = useCallback(async () => {
        ...
        try {
            ...
            // set directly to avoid a missing-deps warning for closeDeleteSheet
                        ...
                        <div key={serveDate} className="orders-list">
                            ...
                            {dayOrders.map((order) => {
                                ...
                                const deadlinePassed =
                                    item.order_deadline &&
                                    new Date(item.order_deadline) < new Date();

                                return (
                                    <article key={order.id} className="order-card">
                                        <div className="order-img-wrapper">
                                            {item.image_url ? (
                                                <img src={item.image_url} alt={item.title} className="order-img" />
                                            ) : (
                                            ...
                                            {deadlinePassed ? (
                                                ...
                                                <>
                                                    <div className="qty-controls">
                                                        <button onClick={() => adjustQuantity(order.id, -1)}>−</button>
                                                        <input
                                                            type="number"
                                                            min="1"
                                                            max="10"
